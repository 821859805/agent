...
class GradioUI:
    ...
    def launch(self, share: bool = False, debug: bool = True, server_port: int = None, **kwargs):
        ...
        try:
            # Check Gradio version for theme parameter placement
            import gradio as gr
            gradio_version = gr.__version__
            try:
                from packaging import version
                is_gradio_6_plus = version.parse(gradio_version) >= version.parse("6.0.0")
            except (ImportError, ValueError):
                version_parts = gradio_version.split('.')
                try:
                    major = int(version_parts[0])
                    is_gradio_6_plus = major >= 6
                except (ValueError, IndexError):
                    is_gradio_6_plus = False
            
            # In Gradio 6.0+, theme should be in launch() not Blocks()
            if is_gradio_6_plus and "theme" not in launch_kwargs:
                launch_kwargs["theme"] = "ocean"
    ...
    def create_app(self):
        ...
        # Check if theme parameter should be in Blocks or launch()
        # In Gradio 6.0+, theme moved to launch() method
        gradio_version = gr.__version__
        try:
            from packaging import version
            is_gradio_6_plus = version.parse(gradio_version) >= version.parse("6.0.0")
        ...
        # Use Blocks with or without theme based on version
        if is_gradio_6_plus:
            demo = gr.Blocks()
        else:
        ...
        with demo:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=3):
                    # Main chat interface - try with type="messages" first, fallback if not supported
                    # Build base kwargs that should work with all versions
                    base_kwargs = {
                        "label": "Agent",
                        "height": 600,
                        "latex_delimiters": [
                    ...
                    # Try to create Chatbot with type="messages" first (Gradio 4.1.0+)
                    # If it fails, fall back to basic Chatbot without type parameter
                    chatbot = None
                    try:
                        # Try with type="messages" for newer Gradio versions
                        chatbot_kwargs = base_kwargs.copy()
                        ...
                        # If type parameter is not supported, use basic Chatbot
                        if "type" in str(e) or "unexpected keyword" in str(e) or "got an unexpected keyword" in str(e):
                            chatbot = gr.Chatbot(**base_kwargs)
                        else:
                            # Re-raise if it's a different error
                            raise
                    
                    # Ensure chatbot was created
                    if chatbot is None:
                        chatbot = gr.Chatbot(**base_kwargs)
